...
topN_pwys <- getTop(InputHumannData.tse, 20, assay.type = params$assayTested)
...
hist_unmapped <- ggplot(data.frame(UNMAPPED = assay(InputHumannData.tse, params$assayTested)["UNMAPPED", ]), aes(x = UNMAPPED)) + 
  geom_histogram(binwidth = 0.01, fill = colors_top2[1], alpha = 0.65)

hist_unintegrated <- ggplot(data.frame(UNINTEGRATED = assay(InputHumannData.tse, params$assayTested)["UNINTEGRATED", ]), aes(x = UNINTEGRATED)) + 
...
plot_unintegr_unmapp <- meltSE(InputHumannData.tse, assay.type = params$assayTested, add.col = c("sex", "donor_microbiome_type")) %>% 
  filter(grepl("^UN", FeatureID)) %>% 
...
pheatmap::pheatmap(assay(InputHumannData.tse, params$assayTested) %>% magrittr::set_colnames(colData(InputHumannData.tse)$sample_name), scale = "row", annotation_col = colData(InputHumannData.tse) %>% as.data.frame() %>%  magrittr::set_rownames(.$sample_name) %>% select(sex, donor_microbiome_type, cage), angle_col = 315, show_rownames = FALSE)
```
...
```{r}
InputHumannData.tse %>% 
  meltSE(assay.type = params$assayTested, add.col = c("donor_microbiome_type")) %>% 
  group_by(FeatureID, donor_microbiome_type) %>% 
  reframe(
    N = n(),
    N_not_0 = sum(!!sym(params$assayTested) > 0),
    mean_abund = mean(!!sym(params$assayTested))
  ) %>% 
